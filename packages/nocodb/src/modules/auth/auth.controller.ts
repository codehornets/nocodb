...
export class AuthController {
  ...
  @Post([
    ...
    '/api/v1/db/auth/user/onboarding',
    '/api/v1/auth/user/onboarding',
    '/api/v2/auth/user/onboarding',
  ])
  @UseGuards(PublicApiLimiterGuard)
  @HttpCode(200)
  async onboarding(@Req() req: NcRequest, @Res() res: Response): Promise<any> {
    if (this.config.get('auth', { infer: true }).disableEmailAuth) {
      NcError.forbidden('Email authentication is disabled');
    }
    res.json(
      await this.usersService.onboarding({
        body: req.body,
        req,
        res,
      }),
